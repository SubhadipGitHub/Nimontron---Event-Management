...
- **Organizer page** — add guests one at a time or via bulk paste (`name, phone, group, side, meal type, plates allotted`), search/filter the guest list, edit event details (couple's names, fun facts, meal/side options), upload one shared invite image/GIF/MP4, set the override PIN, manage all three access codes, copy direct share links per role, and reset all guest/serving data. Also where new events are created.
